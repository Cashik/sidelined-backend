    ...
    async def invoke_tool(self, tool_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with sse_client(url=self.url) as (read, write):
            async with ClientSession(
                ...
                # Initialize the connection
                await session.initialize()
                print(f"Successfully initialized MCP client for invoke tool {tool_name} with params {params}")
                result = await session.call_tool(tool_name, params)
                print(f"Result of invoke tool {tool_name} with params {params}: {result}")
                if result.isError:
